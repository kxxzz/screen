...
void ws_send(void* ws_, struct ws_frame* header) {
	...
	if (header->payloadLength < 126) {
		/* headerSize same */
		payloadField = (int)header->payloadLength;
	...
	if (payloadField == 126) {
		*(uint16_t*)&headerBuffer[2] = htons((u_short)header->payloadLength);
...
#ifdef _WIN32
        WSABUF dataBuf = { headerSize, headerBuffer };
        WSASend(ws->socket, &dataBuf, 1, NULL, MSG_PARTIAL, NULL, NULL);
#else
		send(ws->socket, headerBuffer, headerSize, MSG_MORE);
#endif
		send(ws->socket, header->data, (int)header->payloadLength, 0);
	} else {
...
int _ws_handle_websocket(struct websocket* ws, char* data, int dataLength) {
	...
	return (int)(nextPart + payloadLength - data); /* return processed data count */
}
...
int _ws_handle_upgrade(struct websocket* ws, char* data, int dataLength) {
	...
	while (ws_readLine(&line) != 0) { /* read till empty line */
		char* buffer;
		lineLength = (int)(line.end - line.start);
		
        buffer = malloc(lineLength + 1);
		memcpy(buffer, line.start, lineLength);
        buffer[lineLength] = 0;
		printf("Ok: %s\n", buffer);
		free(buffer);
	...
	return (int)(line.current - data); /* processed byte count */
}
...
int ws_handle(void* ws_, int timeout) {
	...
	ws = ws_;
#ifdef _WIN32
    pollResult = WSAPoll(&ws->fd, 1, timeout);
    if (pollResult > 0) {
        ioctlsocket(ws->socket, FIONREAD, &dataLength);
#else
...
#ifdef _WIN32
    WSABUF dataBuf = { dataLength, data };
    readResult = WSASend(ws->socket, &dataBuf, 1, NULL, 0, NULL, NULL);
#else
